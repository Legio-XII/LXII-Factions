...
if (bnb_e_debug) then {
	if (_fileName isEqualTo "N/A") then {
		systemChat (format["[LXII Debug] %1", _message]);
		diag_log format["[LXII Debug] %1", _message];
	} else {
		systemChat (format["[LXII Debug] [%1] %2", _fileName, _message]);
		diag_log format["[LXII Debug] [%1] %2", _fileName, _message];
	};
};
